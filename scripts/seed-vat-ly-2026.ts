...
async function main() {
  ...
  const mdPath = 'D:/thptqg/2026/De_thi_Vat_Ly_2026.md'
  let mdContent = ''
  ...
  const prov = await prisma.provenance.findFirst({ where: { sourceName: 'Bộ Giáo dục và Đào tạo', attribution: { contains: 'THPT' } } })

  const paper = await prisma.testPaper.create({
    data: {
      slug: 'vat-ly-2026',
      title: 'THPT Quốc gia — Vật Lý, đề chính thức 2026',
      levelId: levelId,
      examId: exam.id,
      year: 2026,
      totalDuration: 50 * 60,
